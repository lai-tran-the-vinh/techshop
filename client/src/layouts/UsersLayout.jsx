...
function Header() {
  ...
  const drawerBranchItems = [
    {
      ...
      children: allBrands.map((branch, index) => (
        <Card className="sm:p-4 flex! gap-8! border-none!">
          <Typography.Title level={5} className="mb-4!">
            {branch.name}
          </Typography.Title>
  ...
  return (
    <Layout.Header
      id="main-header"
      className="print:hidden! font-inter! p-8! lg:px-4! w-full! fixed! top-0! left-0! right-0! z-[99]! lg:p-10! bg-gradient-primary-to-secondary! xl:h-70! flex! flex-col! lg:flex-row! items-center! justify-center! border-b! border-gray-200! h-auto! min-h-[60px]! lg:h-20!"
    >
      <div className="w-full lg:w-5/6 flex items-center justify-between gap-2 lg:gap-4 mb-2 lg:mb-0">
        ...
        <div className="flex items-center gap-4 sm:gap-15">
          {localStorage.getItem('access_token') ? (
            ...
            <Flex
              gap={4}
              className="hidden! lg:flex! p-10! cursor-pointer! rounded-full! border-0! bg-[#7d161c]!"
            >
              <Dropdown
                menu={{
                  items: [
      ...
      <Drawer
        title={
          <Typography.Title
            level={3}
            className="font-bold! m-0! text-lg! text-white!"
          >
            Techshop
          </Typography.Title>
        }
